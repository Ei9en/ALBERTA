...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional NPZ output containing the response-space geometry "
            "needed for the paper figure."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Stop after saving Pareto geometry; do not rewrite the HMI "
            "queue or diagnostics JSON."
        ),
    )
...
def main() -> None:
    ...
    # ========================================================
    # Save response-space geometry for the paper figure
    # ========================================================

    first_front = np.flatnonzero(
        pareto_ranks == 1
    ).astype(np.int32)
    ...
    np.savez_compressed(
        geometry_path,
        predicted_delta_kl=(
            predicted_kl.astype(np.float32)
        ),
        predicted_delta_v=(
            predicted_v.astype(np.float32)
        ),
        first_front_indices=first_front,
        selected_indices=(
            selected.astype(np.int32)
        ),
        learner_epoch=np.asarray(
            args.learner_epoch,
            dtype=np.int32,
        ),
        seed=np.asarray(
    ...
    print()
    print(
        f"Saved Pareto geometry: {geometry_path}"
    )
    print(
        f"  eligible candidates: {len(candidates):,}"
    )
    print(
        f"  first front:         {len(first_front):,}"
    )
    print(
    ...
    if args.geometry_only:
        print()
        print(
            "Geometry-only mode: queue and diagnostics were not rewritten."
        )
        return
